...
import logging
from utils.metrics_util import get_rouge_score
from utils.training_util import FineTuneCallBack
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, Seq2SeqTrainingArguments))#ModelArguments中包含的是关于模型的属性；DataTrainingArguments中包含的是关于微调数据的属性；Seq2SeqTrainingArguments中包含的是模型微调的参数
    config_file = './config/dialogsum_config.json'
    ...
    shutil.copyfile(config_file, os.path.join(output_dir, "run_config.json"))
