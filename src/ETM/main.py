...
parser.add_argument('--rho_size', type=int, default=512, help='dimension of rho')
parser.add_argument('--emb_size', type=int, default=512, help='dimension of embeddings')
...
parser.add_argument('--num_words', type=int, default=50, help='number of words for topic viz')
...
def visualize(m, show_emb=True):
    ...
    queries = ['medical', 'computer', 'sports', 'religion', 'man', 'love',
    ...
    with torch.no_grad():
        ...
        for k in range(args.num_topics):
            ...
            with open(os.path.join(args.save_path, 'topic_words_de.txt'), 'a') as f:
                f.write(str(k))
...
if args.mode == 'train':
    ...
    with torch.no_grad():
        ...
        if args.train_embeddings:
            ...
            os.makedirs(args.save_path, exist_ok=True)
            with open(os.path.join(args.save_path, 'embedding.txt'), 'w') as f:
                f.write(str(rho_etm))
                f.close()
            for word in queries:
