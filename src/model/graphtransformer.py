...
class GraphTransformerMultiHeadAttentionLayer(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.config = config
        self.hid_dim = config.d_model # 1024
        self.n_heads = config.gt_encoder_attention_heads    #=8
        
        assert self.hid_dim % self.n_heads == 0
        self.head_dim = self.hid_dim // self.n_heads    # head_dim=128
        
        self.fc_q = nn.Linear(self.hid_dim,self.hid_dim)    # Linear(in_features=1024,out_features=1024,bias=True)
        self.fc_k = nn.Linear(self.hid_dim,self.hid_dim)    # Linear(in_features=1024,out_features=1024,bias=True)
        self.fc_v = nn.Linear(self.hid_dim,self.hid_dim)    # Linear(in_features=1024,out_features=1024,bias=True)
        
        self.fc_o = nn.Linear(self.hid_dim,self.hid_dim)    # Linear(in_features=1024,out_features=1024,bias=True)
        self.dropout = nn.Dropout(config.gt_attention_dropout)
        self.scale = self.head_dim ** 0.5   #dk
        
        self.feature_maps_num = len(config.feature_types)   # 4种图 feature_maps_num=4
        self.feature_conv = nn.Sequential(*[nn.Conv2d(     # add adj
...
#   (4): Conv2d(4, 1, kernel_size=(1, 1), stride=(1, 1))
#   (5): Conv2d(4, 1, kernel_size=(1, 1), stride=(1, 1))
#   (6): Conv2d(4, 1, kernel_size=(1, 1), stride=(1, 1))
#   (7): Conv2d(4, 1, kernel_size=(1, 1), stride=(1, 1))
# )
        ...
        if 'discourse_adj' in config.feature_types:
            self.discourse_embed = nn.Embedding(len(DISCOURSE_RELATIONS)+1,1,padding_idx=0)# Embedding(6,1,padding_idx=0)
        if 'distance_adj' in config.feature_types:
            self.distance_embed = nn.Embedding(config.max_utt_num*2-1,1)# Embedding(59,1)
    ...
    def forward(self,query,key,value,adj_mats,mask):
        batch_size = query.shape[0]# batch_size:2
        Q = self.fc_q(query) #bs,seq_len,hid_dim
        K = self.fc_k(key)
        V = self.fc_v(value)
        
        Q = Q.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3) #[bs,n_heads,seq_len,hid_dim] (2,8,16,1024)
        K = K.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3)
        V = V.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3)
        # 矩阵相乘
        energy = torch.matmul(Q,K.permute(0,1,3,2)) / self.scale#2,8,16,16
        #energy = [batch size, n heads, query len, key len]
        
        adj_ls = [] #[4个tensor(bs,num_utt,num_utt),..]
        for k,v in adj_mats.items():
            # add adj
            if k=='distance_adj':
                adj_ls.append(self.distance_embed(v).squeeze(-1)) # [bs,num_utt,num_utt] 2,16,16
            elif k=='speaker_adj':
        ...
        energy = self.conv_activation_fn(torch.stack(energy_ls,dim=1)) # bs,n_head,q_len,k_len 2,8,16,16
        ...
        attention = torch.softmax(energy,dim=-1)#2,8,16,16
        ...
        x = torch.matmul(self.dropout(attention),V)#2,8,16,128
        x = x.permute(0,2,1,3).contiguous() #2，16，8，128
        x = x.view(batch_size,-1,self.hid_dim)# 2，16，1024
        x = self.fc_o(x) #2，16，1024
        return x,attention#x 2，16，1024  attention 2,8,16,16
...
class GraphTransformerLayer(BartEncoderLayer):
    ...
    def __init__(self,config):
        ...
        # )
        self.fc1 = nn.Linear(self.embed_dim, config.gt_encoder_ffn_dim)# Linear(in_features=1024, out_features=4096, bias=True)
        self.fc2 = nn.Linear(config.gt_encoder_ffn_dim, self.embed_dim)# Linear(in_features=4096, out_features=1024, bias=True)
    def forward(
        self,
        hidden_states: torch.Tensor,#（2,16,1024）
        attention_mask: torch.Tensor,#(2 1 1 16)
        ...
        **kwargs,
        ...
        """
        adj_mats = kwargs.get('adj_mats',None)#{dict:4}  {'speaker_adj'={tensor:(2,16,16)},.. }
        residual = hidden_states# (2,16,1024)
        hidden_states, attn_weights = self.self_attn(
            hidden_states,hidden_states,hidden_states,
            adj_mats,
            mask=attention_mask,
        )#hidden_states 2，16，1024  attention 2,8,16,16
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)

        residual = hidden_states
        hidden_states = self.activation_fn(self.fc1(hidden_states))# 2,16,4096
        hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
        hidden_states = self.fc2(hidden_states) #2,16,1024
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        ...
        if output_attentions:
            outputs += (attn_weights,)#attn_weights (2,8,16,16)

        return outputs #{(2,16,1024),(2,8,16,16)}
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(self,'embed_positions'):
            ...
            hidden_states = inputs_embeds#(bs,16,hid_dim)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    ...
                    layer_outputs = encoder_layer(#进入GraphTransformerLayer forward
                        hidden_states,
                        attention_mask,
                ...
                hidden_states = layer_outputs[0] #(2,16,1024)

            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],) #{(2,8,16,16),,}
